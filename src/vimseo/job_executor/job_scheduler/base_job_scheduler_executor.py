...
class BaseJobSchedulerExecutor(JobExecutor):
    ...
    def _convergence_source_directory(self) -> Path:
        """Read convergence files from the scheduler-side directory once known.

        Before :meth:`_set_job_scheduler_job_directory` has run, falls back to
        the job directory (matching the base class default).
        """
