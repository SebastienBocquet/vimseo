...
class BaseJobSchedulerExecutor(JobExecutor):
    ...
    def _convergence_source_directory(self) -> Path:
        """The scheduler-side run directory, once known, else the job directory.

        The solver writes its convergence files (``.sta``, ``.msg``, ...) into
        ``job_scheduler_job_directory`` while the job runs there; they only land
        in ``_job_directory`` once :meth:`_copy_back_output_files` runs, at the
        end of the job.
        """
        if self._job_scheduler_job_directory:
            return Path(self._job_scheduler_job_directory)
